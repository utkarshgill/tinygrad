...
base_rewrite = PatternMatcher([
  ...
  (UPat(Ops.STORE, src=(UPat((Ops.DEFINE_REG, Ops.DEFINE_GLOBAL)), UPat()), name="x"), lambda ctx,x: f"{ctx[x.src[0]]} = {ctx[x.src[1]]};"),
  # alu/gep
...
extra_pm = PatternMatcher([
  ...
  (UPat((*GroupOp.ALU, Ops.CAST, Ops.BITCAST, Ops.STORE, Ops.INDEX), dtype=dtypes.bool, name="alu"), no_vectorized_alu),
  ...
  def _render(self, uops:list[UOp]) -> tuple[str, list[str], list[tuple[str,tuple[DType,bool]]]]:
    ...
    for u in uops:
      ...
      if (u.op is not Ops.CAST or u.dtype.vcount == 1) and (u.op in {Ops.CONST, Ops.GEP, Ops.INDEX, Ops.CUSTOMI} or \
        ...
        if u.op in {Ops.RANGE, Ops.STORE, Ops.DEFINE_LOCAL} or u.dtype == dtypes.void:
          if u.op is Ops.STORE and len(u.src) > 0 and u.src[0].op in {Ops.DEFINE_REG, Ops.DEFINE_GLOBAL}: r[u] = r[u.src[0]]  # register assignment
        else:
