...
class UOp(MathTrait, metaclass=UOpMetaClass):
  ...
  def store(self, *src:UOp, **kwargs): 
    # For unified STORE: preserve target dtype for register assignment, use dtypes.void for memory operations
    store_dtype = self.dtype if self.op in {Ops.DEFINE_REG, Ops.DEFINE_GLOBAL} else dtypes.void
    return UOp(Ops.STORE, store_dtype, (self,)+src, **kwargs)
  ...
  def assign(self, x:UOp): return UOp(Ops.STORE, self.dtype, (self,x))
...
class UPat(MathTrait):
  ...
  def assign(self, x:UPat, **kwargs): return UPat(Ops.STORE, self.dtype, (self,x), **kwargs)
